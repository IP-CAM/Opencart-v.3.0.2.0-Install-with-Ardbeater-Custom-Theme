...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				if($this->config->get('module_point_system_status')) {
				
					// Input reward points saat pesanan selesai
					if(in_array($this->request->post['order_status_id'], $this->config->get('config_complete_status'))) {
						...
						if($query->row['value'] > 0) {
							$total = floor($query->row['value']);
							$points_reward = ($total/5000)*$this->config->get('module_point_system_reward');
							$this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET customer_id = '" . (int)$order_info['customer_id'] . "', order_id = '" . (int)$order_id . "', description = 'Reward points from total orders', points = '" . $points_reward . "', date_added = NOW()");
						}							
					}
					...
					if($this->config->get('config_fraud_status_id') == $this->request->post['order_status_id']) {
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_reward WHERE customer_id = '" . (int)$order_info['customer_id'] . "' AND description = 'Reward points from total orders' AND order_id='" . (int)$order_id . "'");
						
						if($query->num_rows >= 1) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "customer_reward SET customer_id = '" . (int)$order_info['customer_id'] . "', order_id = '" . (int)$order_id . "', description = 'Reverse reward points from total orders', points = '-" . (float)$query->row['points'] . "', date_added = NOW()");
						}							
					}						
				}
